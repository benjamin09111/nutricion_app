...
export default function AdminFeedbackPage() {
    ...
    const handleResolve = async (id: string) => {
        try {
            const token = localStorage.getItem('auth_token');
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/support/${id}/resolve`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/support/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            ...
            case 'COMPLAINT': return <AlertTriangle className="w-4 h-4 text-rose-600" />;
            case 'IDEA': return <Lightbulb className="w-4 h-4 text-amber-600" />;
            case 'PASSWORD_RESET': return <User className="w-4 h-4 text-indigo-600" />;
            case 'CONTACT': return <Mail className="w-4 h-4 text-blue-600" />;
    ...
    const getTypeLabel = (type: string) => {
        switch (type) {
            ...
            case 'COMPLAINT': return 'Problema';
            case 'IDEA': return 'Idea';
            case 'PASSWORD_RESET': return 'Reset Pass';
    ...
    const getTypeColor = (type: string) => {
        switch (type) {
            ...
            case 'COMPLAINT': return 'bg-rose-50 text-rose-700 border-rose-200';
            case 'IDEA': return 'bg-amber-50 text-amber-700 border-amber-200';
            case 'PASSWORD_RESET': return 'bg-indigo-50 text-indigo-700 border-indigo-200';
                            ...
                            <div key={item.id} className="p-4 sm:p-6 hover:bg-slate-50 transition-colors group">
                                <div className="flex flex-col sm:flex-row gap-4 items-start justify-between">
                                    <div className="flex gap-4 w-full">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 mt-1 ${getTypeColor(item.type).split(' ')[0]
                                            }`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${getTypeColor(item.type)}`}>
                                            ...
                                            <p className="text-slate-600 text-sm whitespace-pre-wrap wrap-break-word leading-relaxed">
                                                {item.message || <span className="italic text-slate-400">Sin mensaje</span>}
                                            </p>
                                        </div>
                                    </div>

    ...
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-8 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-6 dark:border-slate-800">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-600 dark:border-indigo-900/50 dark:bg-indigo-950/50 dark:text-indigo-400">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-xs font-bold uppercase tracking-wider text-white shadow-lg transition-transform active:scale-95 hover:bg-indigo-700 shrink-0"
          >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          {/* Tabs */}
          <div className="flex items-center gap-2 rounded-2xl bg-slate-200/60 p-1.5 dark:bg-slate-900">
            <button
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-64 animate-pulse rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"
              />
            ))}
          </div>
          ...
          <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
            ...
            <button
              onClick={handleOpenCreate}
              className="mt-6 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-xs font-bold text-white hover:bg-indigo-700"
            >
              <Plus className="h-4 w-4" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTestimonials.map((t) => (
              <div
                key={t.id}
                className="relative rounded-3xl border border-slate-200 bg-white p-6 shadow-md transition-all hover:shadow-xl dark:border-slate-800 dark:bg-slate-900 flex flex-col justify-between"
              >
                <div className="space-y-4">
                  ...
                  {t.highlight && (
                    <div className="inline-block rounded-full bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
                      ⚡ {t.highlight}
                    </div>
                  )}

                  {/* Quote Body */}
                  <p className="text-xs text-slate-700 leading-relaxed italic dark:text-slate-300">
                    "{t.quote}"
                  </p>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-2">
                    ...
                    <button
                      ...
                      className="rounded-xl border border-slate-200 p-2 text-slate-600 hover:bg-slate-100 hover:text-indigo-600 dark:border-slate-800 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-indigo-400"
                    ...
                    {!t.isReviewed && (
                      <button
                        ...
                        className="rounded-xl border border-amber-200 bg-amber-50 p-2 text-amber-600 hover:bg-amber-100 dark:border-amber-900/50 dark:bg-amber-950/50 dark:text-amber-400"
                    ...
                    <button
                      ...
                      className="rounded-xl border border-slate-200 p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600 dark:border-slate-800 dark:hover:bg-rose-950/50 dark:hover:text-rose-400"

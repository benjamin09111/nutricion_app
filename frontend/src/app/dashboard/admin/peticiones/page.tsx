...
export default function PeticionesPage() {
    ...
    const fetchRequests = async (pageArg = page) => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/requests?${queryParams.toString()}`, {
                    ...
                    >
                        ...
                        <span className={cn(
                            ...
                            {tab.id === 'PENDING' ? counts.pending :
                                tab.id === 'ALL_ACCEPTED' ? counts.accepted :
                                    counts.rejected}
                        </span>
                    </button>
                ))}
                    ...
                    {!isLoading && totalPages > 1 && (
                        <div className="flex items-center justify-between border-t border-slate-200 pt-4 mt-auto">
                            <p className="text-sm text-slate-500">
                                Mostrando <span className="font-bold">{requests.length}</span> de <span className="font-bold">{totalItems}</span> registros
                            </p>
                                        ...
                                        <div className="flex flex-col gap-3 p-4 rounded-xl bg-slate-900 text-white shadow-lg border-l-4 border-indigo-500 relative overflow-hidden">
                                            <div className="flex items-center gap-4 z-10">
                                                ...
                                                <div className="text-sm flex-1">
                                                    ...
                                                    <p className="text-slate-200 text-xs leading-tight">Verifica el registro en la Superintendencia de Salud.</p>
                                                </div>
                                            </div>

                                            <div className="pt-2 border-t border-white/10 flex flex-col gap-2 z-10">
                                                <p className="text-[10px] text-indigo-200">
                                                    Al hacer clic en "Validar", se copiará automáticamente el nombre: <span className="text-white font-bold">&quot;{selectedRequest.fullName}&quot;</span>
                                                </p>
                                                <button
                                                    onClick={() => {
                                                        navigator.clipboard.writeText(selectedRequest.fullName);

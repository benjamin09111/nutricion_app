...
export function CreateRecipeModal({ isOpen, onClose, onSuccess }: CreateRecipeModalProps) {
    const { register, control, handleSubmit, setValue, watch, reset, formState: { errors, isSubmitting } } = useForm<CreateRecipeForm>({
        defaultValues: {
            portions: 1,
            ingredients: []
        }
    ...
    // Calculate Macros
    const totals = watchedIngredients.reduce((acc, curr) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Crear Nuevo Plato" className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-700">Nombre del Plato</label>
                        <Input {...register('name', { required: 'El nombre es obligatorio' })} placeholder="Ej: Cazuela de Vacuno" />
                        {errors.name && <span className="text-xs text-red-500">{errors.name.message}</span>}
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-700">Porciones (Rendimiento)</label>
                        <Input
                            type="number"
                ...
                <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700">Descripción / Preparación</label>
                    <textarea
                ...
                <div className="space-y-2 relative">
                    <label className="text-sm font-medium text-slate-700">Agregar Ingredientes</label>
                    ...
                    {searchTerm.length >= 2 && (
                        <div className="absolute z-10 w-full bg-white border border-slate-200 rounded-lg shadow-lg mt-1 max-h-60 overflow-y-auto">
                            {isSearching ? (
                                ...
                                <div className="p-3 text-sm text-slate-500 text-center">No se encontraron ingredientes</div>
                            )}
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-3">
                    <h4 className="text-sm font-medium text-slate-700 flex justify-between">
                        ...
                        <span className="text-xs font-normal text-slate-500">{fields.length} items</span>
                    </h4>

                    {fields.length === 0 ? (
                        <div className="text-center py-8 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                            <p className="text-sm text-slate-400">Busca y agrega ingredientes arriba</p>
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {fields.map((field, index) => (
                                <div key={field.id} className="flex flex-col gap-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        ...
                                        <div>
                                            <label className="text-[10px] text-slate-500">Marca (Opcional)</label>
                                            <Input
                                                className="h-8 text-xs bg-white"
                                                placeholder="Ej: Lider..."
                                                {...register(`ingredients.${index}.brandSuggestion` as const)}
                                            />
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="bg-slate-900 text-white p-4 rounded-xl shadow-lg">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h4 className="font-semibold text-sm">Información Nutricional (por porción)</h4>
                    </div>
                    <div className="grid grid-cols-4 gap-4 text-center mb-4">
                        <div>
                            <div className="text-2xl font-bold text-emerald-400">{Math.round(perPortion.calories)}</div>
                            <div className="text-[10px] uppercase tracking-wider text-slate-400">Kcal</div>
                        </div>
                        <div>
                            <div className="text-lg font-bold">{Math.round(perPortion.proteins)}g</div>
                            <div className="text-[10px] uppercase tracking-wider text-slate-400">Prot</div>
                        </div>
                        <div>
                            <div className="text-lg font-bold">{Math.round(perPortion.carbs)}g</div>
                            <div className="text-[10px] uppercase tracking-wider text-slate-400">Carb</div>
                        </div>
                        <div>
                            <div className="text-lg font-bold">{Math.round(perPortion.lipids)}g</div>
                            <div className="text-[10px] uppercase tracking-wider text-slate-400">Grasas</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3 flex items-start gap-2">
                        ...
                        <p className="text-xs text-orange-200">
                            <strong>Nota Importante:</strong> Sumar los ingredientes no es lo mismo que el resultado final, ¡ya que un plato tiene preparación (cocción, pérdida de agua, absorción de aceite)!
                        </p>
                    </div>
                </div>

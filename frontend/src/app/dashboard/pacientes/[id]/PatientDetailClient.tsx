              ...
              <>
                <button
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-xl border border-slate-100 shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-40 py-1">
                      <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-5">
        <div className="flex gap-0 border-b border-slate-100 overflow-x-auto overflow-y-hidden no-scrollbar">
          {tabs.map((tab) => (
            <button
              key={tab.label}
              onClick={() => { if (!tab.disabled) state.setActiveTab(tab.label as TabType); }}
              disabled={tab.disabled}
              className={cn(
                "px-4 py-2.5 text-[11px] font-black uppercase tracking-wider transition-all duration-200 whitespace-nowrap border-b-2 -mb-px cursor-pointer shrink-0",
                tab.disabled

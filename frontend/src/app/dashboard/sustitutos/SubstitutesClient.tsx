...
export function SubstitutesClient() {
  ...
  const handleChange = (
    id: string,
    field: keyof SubstituteEntry,
    value: string,
  ) => {
    setSubstitutes(
      substitutes.map((s) => (s.id === id ? { ...s, [field]: value } : s)),
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          "Conocimiento guardado correctamente. Tu IA ya conoce estos cambios.",
        );
      } else {
        ...
        <ModuleFooter>
          <div className="flex items-center gap-2 text-slate-400">
            ...
            <p className="text-xs font-medium">
              Esta información alimenta al Chatbot de tus pacientes.
            </p>
          </div>
      ...
      <div className="max-w-4xl mx-auto space-y-8 mt-6">
        <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-3xl flex items-start gap-4">
          ...
          <div>
            <h3 className="text-sm font-black text-indigo-900 uppercase tracking-widest mb-1">
              Entrenamiento Prioritario
            </h3>
            <p className="text-xs text-indigo-700 leading-relaxed font-medium">
              Los sustitutos que definas aquí serán la fuente de verdad
              principal para el Chatbot. Si un paciente tiene dudas sobre qué
              alimentos intercambiar, la IA responderá basándose en estas
              reglas.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4 pb-20">
          <div className="flex items-center justify-between px-4">
            ...
            <Button
              variant="ghost"
              className="text-indigo-600 font-bold text-xs gap-2 hover:bg-indigo-50"
              onClick={handleAdd}
            >
                  ...
                  >
                    <td className="px-6 py-2">
                      <Input
                        ...
                        value={item.original}
                        onChange={(e) =>
                          handleChange(item.id, "original", e.target.value)
                        }
                    ...
                    <td className="px-6 py-2">
                      <Input
                        ...
                        onChange={(e) =>
                          handleChange(item.id, "substitute", e.target.value)
                        }
                        className="h-10 border-transparent bg-transparent hover:border-slate-200 focus:bg-white rounded-xl font-bold text-sm"
                      />
                    </td>
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-500">
                          Define tus sustitutos frecuentes para mejorar la IA.
                        </p>
                      </div>
                      <Button
                        onClick={handleAdd}

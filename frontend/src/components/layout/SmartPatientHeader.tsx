    ...
    return (
        <div
            ...
            className={cn(
                ...
                patient ? "bg-emerald-50 border-b border-emerald-100" : "bg-red-50 border-b border-red-100 hover:bg-red-100"
            )}
        >
            <div className="flex items-center gap-3">
                <div className={cn("p-1.5 rounded-full", patient ? "bg-emerald-200" : "bg-red-200")}>
                    {patient ? <CheckCircle2 className="h-3 w-3 text-emerald-800" /> : <AlertCircle className="h-3 w-3 text-red-800" />}
                </div>
                {patient ? (
                    <p className="text-xs font-bold text-emerald-900">
                        Paciente Seleccionado: <span className="font-black">{patient.name}</span>

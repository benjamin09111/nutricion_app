...
export function PremiumGuard({ children, feature = 'canGenerateDiet', fallback, showLockIcon = true }: PremiumGuardProps) {
    ...
    const handleLockedClick = (e: React.MouseEvent) => {
        ...
        toast("Funcionalidad Pro", {
            description: "Esta característica requiere el plan de pago. ¿Quieres subir de plan ahora?",
            action: {
                label: "¡Subir a Pro!",
    ...
    return (
        <div className="relative group cursor-not-allowed" onClick={handleLockedClick}>
            <div className="pointer-events-none opacity-50 grayscale select-none" aria-disabled="true">
                {children}
            </div>
...
export function PremiumOverlay({ message = "Esta sección es exclusiva para suscriptores Pro" }: { message?: string }) {
    return (
        <div className="absolute inset-0 bg-white/40 backdrop-blur-[4px] z-10 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
            <div className="bg-white p-8 rounded-[2rem] shadow-[0_20px_50px_rgba(79,70,229,0.15)] border border-indigo-100 max-w-sm w-full transform hover:scale-[1.01] transition-transform">
                ...
                <h3 className="text-xl font-black text-slate-900 mb-2 uppercase tracking-tight">Acceso Pro Requerido</h3>

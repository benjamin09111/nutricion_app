...
export default function LoginForm() {
    ...
    // Modal States
    const [activeModal, setActiveModal] = useState<'reset' | 'contact' | null>(null);
    ...
    const onSubmit = async (data: LoginFormData) => {
        ...
        try {
            ...
            if (['ADMIN', 'ADMIN_MASTER', 'ADMIN_GENERAL'].includes(response.user?.role)) {
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center">
                        <input
                            ...
                            {...register('rememberMe')}
                        />
                        <label htmlFor="rememberMe" className="ml-3 block text-sm leading-6 text-slate-900 cursor-pointer">
            ...
            {activeModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden p-6 relative">
                        ...
                        <div className="mb-6">
                            <div className={`h-10 w-10 rounded-full flex items-center justify-center mb-3 ${activeModal === 'reset' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'
                                }`}>
                                {activeModal === 'reset' ? <KeyRound className="h-5 w-5" /> : <MessageCircle className="h-5 w-5" />}
                            </div>
                            <h3 className="text-lg font-semibold text-slate-900">
                                {activeModal === 'reset' ? 'Recuperar Acceso' : 'Contactar Soporte'}
                            </h3>

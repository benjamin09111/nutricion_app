...
export class IngredientGroupsService {
  ...
  async create(nutritionistId: string, createDto: CreateIngredientGroupDto) {
    ...
    const tagRecords =
      tags && tags.length > 0
        ? await Promise.all(
            tags.map((name: string) => this.getOrCreateTag(name)),
          )
        : [];
  ...
  private async validateGroupOwnership(id: string, nutritionistId: string) {
    if (!nutritionistId)
      throw new ForbiddenException('Nutritionist profile not found');
  ...
  async update(
    ...
    updateDto: CreateIngredientGroupDto,
  ) {
  ...
  async removeIngredients(
    ...
    const deleteMany: Prisma.IngredientGroupEntryScalarWhereInput =
      isRecipeGroup
        ? { recipeId: { in: itemIds } }

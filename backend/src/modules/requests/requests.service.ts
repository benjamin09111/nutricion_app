...
export class RequestsService {
  ...
  async create(createDto: CreateRegistrationRequestDto) {
    ...
    if (existingRequest) {
      throw new BadRequestException(
        'Ya tienes una solicitud pendiente. Te contactaremos pronto.',
      );
    }
    ...
    // 3. Notify admin and confirm receipt to the requester
    this.mailService
      ...
      .catch((err) =>
        console.error('Error sending registration admin notification:', err),
      );
    ...
    return {
      success: true,
      message:
        'Solicitud enviada correctamente. Revisaremos tus datos y te contactaremos.',
    };
  }
  ...
  async findAll(
    ...
    const [data, total, pendingCount, acceptedCount, rejectedCount] =
      await Promise.all([
  ...
  async updateStatus(
    ...
    if (isApproving && !wasAlreadyApproved) {
      ...
      if (existingUser) {
        ...
        return {
          success: true,
          message: 'La cuenta ya existía. Solicitud marcada como aceptada.',
        };
      }
      ...
      try {
        ...
        return {
          success: true,
          message:
            'Solicitud aceptada y credenciales enviadas automáticamente al correo.',
        };
      } catch (error: any) {

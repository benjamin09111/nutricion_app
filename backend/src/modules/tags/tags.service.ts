...
export class TagsService {
  ...
  async remove(id: string, nutritionistId: string, role?: string) {
    ...
    if (!tag) {
      throw new Error(
        'La restricción que intentas eliminar no existe o ya fue borrada.',
      );
    }
    ...
    if (
      !isAdmin &&
      tag.nutritionistId &&
      tag.nutritionistId !== nutritionistId
    ) {
    ...
    if (!isAdmin && !tag.nutritionistId) {
      throw new Error(
        'Las restricciones globales del sistema no pueden ser eliminadas',
      );
    }

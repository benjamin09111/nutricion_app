...
export class RecipeMatchingService {
  ...
  private normalizeList(value: unknown): string[] {
    ...
    return Array.from(
      new Set(
        value
          .map((item) =>
            typeof item === 'string' ? normalizeFoodName(item) : '',
          )
          .filter(Boolean),
      ),
    );
  ...
  async findCompatibleRecipes(
    ...
    const normalizedInput = Array.from(
      new Set(ingredientNames.map((n) => normalizeFoodName(n)).filter(Boolean)),
    );
    ...
    for (const recipe of recipes) {
      ...
      const candidateMainIngredients =
        relationMainIngredients.length > 0
          ? relationMainIngredients.map((ri: any) =>
              normalizeFoodName(ri.ingredient?.name || ''),
            )
      ...
      for (const recipeIngredient of candidateMainIngredients) {
        ...
        for (const input of normalizedInput) {
          if (
            input === normalizedRecipeReq ||
            input.includes(normalizedRecipeReq) ||
            normalizedRecipeReq.includes(input)
          ) {
            hasMatch = true;
            break;
          }
      ...
      const matchPercentage =
        (matchCount / candidateMainIngredients.length) * 100;
    ...
    return scoredRecipes
      ...
      .map((sr) => ({
        ...
        isAdopted:
          sr.recipe.nutritionistId !== nutritionistId &&
          Array.isArray(sr.recipe.savedBy) &&
          sr.recipe.savedBy.length > 0,
      }));
  }
}

...
export class FoodsService {
  ...
  private parseDraftIngredientField(rawValue?: string | null) {
    ...
    const cleaned = value.replace(this.draftMarker, '').trim();
  ...
  private buildDraftIngredientField(
    ...
    const cleaned =
      typeof baseIngredients === 'string' ? baseIngredients.trim() : '';

    if (isDraft) {
      return cleaned ? `${this.draftMarker}\n${cleaned}` : this.draftMarker;
    }
  ...
  private resolveIngredientNotesForWrite(params: {
    ...
    const { cleanedIngredients, isDraft: existingIsDraft } =
      this.parseDraftIngredientField(params.existingIngredients);
  ...
  private assertIngredientOwnership(ingredient: any, nutritionistId?: string) {
    if (!nutritionistId || ingredient.nutritionistId !== nutritionistId) {
      throw new ForbiddenException(
        'No tienes permisos para modificar este ingrediente',
      );
    }
  }
  ...
  private async findDuplicateIngredient(params: {
    ...
    return (this.prisma as any).ingredient.findFirst({
      where: {
        ...(params.excludeIngredientId
          ? { id: { not: params.excludeIngredientId } }
          : {}),
        name: {
  ...
  async create(createFoodDto: CreateFoodDto, userId: string) {
    const { brand, category, tags, isPublic, isDraft, ingredients, ...rest } =
      createFoodDto;
    ...
    const ingredient = await (this.prisma as any).$transaction(
      async (tx: any) => {
    ...
    switch (tab) {
      ...
      case 'favorites':
        ...
        preferenceIngredientIds = (
          ...
          (preference: { ingredientId: string }) => preference.ingredientId,
        );
        if (shouldDebug) {
          this.logger.log(
        ...
        OR: [
          {
            tags: {
              some: {
                name: { equals: tag, mode: Prisma.QueryMode.insensitive },
              },
            },
          },
          ...(nutritionistId
  ...
  async togglePreference(
    ...
    if (!nutritionist) {
      ...
      if (!account) {
        this.logger.error(
          `[togglePreference] Account not found accountId=${userId}`,
        );
      ...
      try {
        ...
        this.logger.error(
          `[togglePreference] Failed auto-create for accountId=${userId}`,
          createError,
        );
        throw new NotFoundException(
          'No se pudo crear el perfil de nutricionista necesario.',
        );
      }
    }
    ...
    if (!ingredient) {
      this.logger.error(
        `[togglePreference] Ingredient not found ingredientId=${ingredientId}`,
      );
    ...
    try {
      ...
      this.logger.error(
        `[togglePreference] Error upserting ingredientId=${ingredientId}`,
        error,
      );
      throw error;
    }
  ...
  async findOne(id: string, requesterAccountId: string) {
    ...
    const nutritionistId = nutritionist?.id;
    const ingredient = await this.findIngredientWithRelations(
      id,
      nutritionistId,
    );
    ...
    if (!canAccess) {
      throw new ForbiddenException(
        'No tienes permisos para ver este ingrediente',
      );
    }
  ...
  async update(
    ...
    const nutritionist = await this.resolveNutritionist(requesterAccountId);
    const nutritionistId = nutritionist?.id;
    const existing = await (this.prisma as any).ingredient.findUnique({
      where: { id },
    });
    ...
    const { brand, category, tags, ingredients, isDraft, ...rest } =
      updateFoodDto;
    const normalizedName =
      rest.name !== undefined
        ? this.normalizeText(rest.name)
        : this.normalizeText(existing.name);
    ...
    try {
      ...
      return rows.map((row) => {
        ...
        return {
          ...
          precioPromedio:
            parseFloat(String(record['Precio promedio']).replace(',', '.')) ||
            0,
        };
      });
    } catch (error) {
